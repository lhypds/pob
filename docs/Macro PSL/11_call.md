...
`logs/<session>/main.macro.psl` is the macro, as it has always been. A called file is not copied into
the session; what the run made of it is in the log and in the slots it filled.
