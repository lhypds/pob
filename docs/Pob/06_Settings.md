...
| `once_interval` | `1000` | Milliseconds a [`once`](../Macro%20PSL/09_once%20blocks.md) block waits between the screenshots it compares (`100` at the least). It costs one screen capture per interval and nothing else — the model is asked only once the picture has actually changed — so it trades how soon a change is acted on against how much of this machine goes on watching for one |
| `once_change_percent` | `0.1` | How much of the picture a [`once`](../Macro%20PSL/09_once%20blocks.md) block has to see differ before it asks its condition, as a percentage of the pixels (`0`–`100`, clamped). `0.1` is about a 45×45 patch of a 1920×1080 screen. Lower is more sensitive — `0.01` acts on a patch a tenth that size — and higher is less: a screen redrawing a graph every second wants more than one showing a still page of text. Written with the sign or without it, so `0.1`, `"0.1%"` and `"10%"` are all read |
| `launch_gap` | `40` | How much of the content area a [`launch()`](../Macro%20PSL/15_launch.md) leaves around the window it puts in it, on every side, in screenshot pixels. Nothing about the frame needs it and the desktop underneath does: a window whose edges sit exactly on Pob's own edges is a window every modern window manager wants to help you line up with, and Pob dragged over one it had fitted perfectly is dragged through its own magnetism. A gap has to clear the distance the desktop reaches across to be worth anything, which is why the default is not the smallest number that shows. `0` fits the window exactly, which is what to set on a desktop that does not do this |
...
| `stop_hook` | — | Shell command to run when a macro runs to its end (e.g. `afplay /System/Library/Sounds/Morse.aiff`). A stopped run does not fire it. The same command written on a line of the macro instead is [`run()`](../Macro%20PSL/14_run.md), which waits for it where it stands |
