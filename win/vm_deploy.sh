...
# (see docs/VM.md). Builds the zip with build_docker.sh, starts the VM headless
# if it is down, copies the zip in over SSH, installs it, and restarts the app
...
#
# Assumes the one-time guest setup in docs/VM.md: autologon, no lock/sleep,
...
if [ ! -e "$VM" ]; then
    echo "❌ No VM at $VM — create it first (docs/VM.md), or set POB_VM."
    exit 1
fi
...
if [ "$SSH_READY" != "1" ]; then
    ...
    echo "   Check the sshd service and administrators_authorized_keys (docs/VM.md)."
    exit 1
fi
